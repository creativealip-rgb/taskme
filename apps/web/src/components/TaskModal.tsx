...
import { TaskStatus, TaskPriority, Task, TaskFormData } from '../types/task';
...
interface TaskModalProps {
  ...
  onSave: (_taskData: TaskFormData) => void;
  task?: Task | null;
...
const getInitialFormData = (task?: Task | null): TaskFormData => {
  if (task) {
    return {
      title: task.title,
...
export const TaskModal = ({ isOpen, onClose, onSave, task = null, apiErrors }: TaskModalProps) => {
  ...
  // Validate entire form for real-time feedback
              ...
              <div className="flex-1">
                ...
                <ul className="mt-1 text-xs text-rose-600 space-y-1">
                  ...
                  {Object.keys(errors).length > 3 && (
                    <li className="text-rose-500">and {Object.keys(errors).length - 3} more...</li>
                  )}
                </ul>
              </div>
            </div>
