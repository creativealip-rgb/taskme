...
import { Task, Workspace, TaskStatus } from '../types/task';
...
export function PublicWorkspace() {
  ...
  useEffect(() => {
    const fetchPublicWorkspace = async () => {
      ...
      try {
        setLoading(true);
        const response = await workspacesApi.getPublicByToken(token);
        setWorkspace(response.data.workspace);
        setTasks(response.data.tasks);
        setError(null);
      } catch (err) {
        console.error('Failed to fetch public workspace:', err);
        setError('Workspace not found or is no longer shared');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-gray-600 mb-6">{error || 'This share link may be invalid or expired.'}</p>
          <a
            href="/"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <a
                href="/"
                className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-xl hover:from-blue-600 hover:to-purple-700 transition-all shadow-lg shadow-blue-500/25 font-medium text-sm"
              >
                Create Your Own
              </a>
            </div>
